...
# function to run a query on the Uniswap GraphQL API
def run_query(q):
...
# function to use a honeypot ethereum checker
...
# function to query Etherscan for the source code of a contract
...
# function to check if a given list of functions exists in the source code of a contract
...
# function to check if a given token already exists in a list of tokens
...
# function to check if the contract is a Uniswap V2 contract
...
# function to get the latest added pairs to the Uniswap network
def get_new_tokens():
    query = """
    {
...
while True:
    ...
    for token in new_tokens:
        ...
        # Saving the output in json files
